...
from . import LOGS, bot, PLUGIN_CHANNEL_ID, CMD_HELP, LANGUAGE, FİREQANQ_VERSION, PATTERNS
...
def extractCommands(file):
    ...
    if re.search(r'CmdHelp\(.*\)', FileRead):
        ...
        for Command in Pattern:
            ...
            # FireqanqPY
            Fireqanqpy = re.search('\"\"\"FİREQANQPY(.*)\"\"\"', FileRead, re.DOTALL)
            if not Fireqanqpy == None:
                Fireqanqpy = Fireqanqpy.group(0)
                for Satir in Fireqanqpy.splitlines():
